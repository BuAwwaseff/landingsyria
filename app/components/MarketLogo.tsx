...
const MARKET_LOGO_VIEWBOX = "0 0 533 94";

type MarketLogoSize = "nav" | "footer";
type MarketLogoVariant = "slot";
...
const frameWidthBySize = {
  nav: "150px",
  footer: "182px",
} as const satisfies Record<MarketLogoSize, string>;
...
export function MarketLogo({
  ...
  const size: MarketLogoSize = preset === "header" ? "nav" : "footer";
  const variant: MarketLogoVariant = "slot";
  ...
  return (
    ...
    >
      <span aria-hidden="true" className="market-logo__shine" />

      <svg
        aria-label={ariaLabel}
        className="market-logo__svg"
        focusable="false"
        preserveAspectRatio="xMidYMid meet"
        role="img"
        shapeRendering="geometricPrecision"
        viewBox={MARKET_LOGO_VIEWBOX}
      ...
      >
        {letters.map((letter, index) => (
          <g
            ...
            className={joinClasses(
              "market-logo__letter",
              `market-logo__letter--${letter.classSuffix}`,
              `market-logo__letter--${letter.tone}`,
            )}
